		    ...
		    */
			...
			public void swicthToWindow(WebDriver driver , String partailWindowTitle) {
				  /*get all window session ID*/
				   ...
				   String parentwinid=it.next();//parent winid
				  // while(it.hasNext()) {//parent id
					   String winID = it.next();//child winid
					   driver.switchTo().window(winID);//switch from parent to child win
